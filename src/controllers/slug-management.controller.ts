...
export const updateRestaurantSlug = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    logger.info(`Restaurant slug updated: ${oldSlug} → ${validatedData.slug} (ID: ${restaurant._id})`);
...
export const checkSlugAvailability = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (!/^[a-z0-9-]{3,50}$/.test(slug)) {
      res.status(400).json({
        error: { 
          message: 'Invalid slug format. Use only lowercase letters, numbers, and hyphens (3-50 characters)',
          available: false
        }
      });
      return;
